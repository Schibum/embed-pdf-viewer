...
if [ "$DRY_RUN" = "true" ]; then
  ...
  while IFS=$'\t' read -r name ver dir; do
    ...
    PIDS+=($!)
    # Throttle
    while [ "$(jobs -r | wc -l)" -ge "$CONCURRENCY" ]; do wait -n 2>/dev/null || sleep 0.1; done
  done <<< "$PKGS"
  ...
  NEED_PUBLISH=""
  while IFS=$'\t' read -r name ver dir; do
    echo "$ALREADY_PUBLISHED" | grep -qx "$name" || NEED_PUBLISH="${NEED_PUBLISH}${name}	${ver}	${dir}"$'\n'
  done <<< "$PKGS"
  ...
  if [ "$TOTAL" -eq 0 ]; then
    ...
    while true; do
      ...
      FAIL_DIR=$(mktemp -d)
      PUB_PIDS=()
      REST=$(echo "$NEED_PUBLISH" | tail -n +2)
      while IFS=$'\t' read -r name ver dir; do
        [ -z "$name" ] && continue
        (
          ...
          if [ $RC -eq 0 ]; then
            ...
            touch "$FAIL_DIR/$(echo "$name" | tr '/' '_')"
          fi
        ) &
        PUB_PIDS+=($!)
        while [ "$(jobs -r | wc -l)" -ge "$CONCURRENCY" ]; do wait -n 2>/dev/null || sleep 0.1; done
      done <<< "$REST"
      for pid in "${PUB_PIDS[@]}"; do wait "$pid" 2>/dev/null || true; done
